<template>
	<view :class="disabled ? 'uni-list-item--disabled' : ''" :hover-class="disabled || showSwitch ? '' : 'uni-list-item--hover'" class="uni-list-item" @click="onClick">
		<view class="uni-list-item__container">
			...
			<!-- 只显示 -->
			<view v-if="!isInput && fromParentValue">
				{{fromParentValue}}
			</view>
			<!-- 输入 -->
			<view v-show="isInput" class="from-parent-input">
				<input type="text" v-model="inputValue" @input="inputTap($event)" />
			</view>
			<view v-if="(showBadge || showArrow || showSwitch) && !isInput" class="uni-list-item__extra">
				<uni-badge v-if="showBadge" :type="badgeType" :text="badgeText" />
				<switch v-if="showSwitch" color="#1296db" :disabled="disabled" :checked="switchChecked" @change="onSwitchChange" />
...
<script>
	...
	export default {
		...
		created() {
			this.inputValue = this.fromParentValue
		},
		methods: {
			inputTap(e) {
